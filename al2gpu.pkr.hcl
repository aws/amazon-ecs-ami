...
build {
  ...
  provisioner "file" {
    source      = "files/repos/amzn2-nvidia.repo"
    destination = "/tmp/amzn2-nvidia.repo"
  }

  provisioner "shell" {
    inline_shebang = "/bin/sh -ex"
    inline = [
      "sudo mv /tmp/amzn2-nvidia.repo /etc/yum.repos.d/amzn2-nvidia.repo"
    ]
  }
  ...
  provisioner "shell" {
    inline_shebang = "/bin/sh -ex"
    inline = [
      "sudo yum install -y kernel-devel-`uname -r` system-release-nvidia nvidia-driver-latest-dkms kmod-nvidia-latest-dkms "
    ]
  }
  ...
  provisioner "shell" {
    inline_shebang = "/bin/sh -ex"
    inline = [
      "sudo yum install -y pciutils xorg-x11-server-Xorg"
    ]
  }
  ...
  provisioner "shell" {
    inline_shebang = "/bin/sh -ex"
    inline = [
      "sudo yum install -y docker-runtime-nvidia oci-add-hooks libnvidia-container libnvidia-container-tools nvidia-container-runtime-hook"
    ]
  }
  ...
  provisioner "shell" {
    inline_shebang = "/bin/sh -ex"
    inline = [
      "sudo yum install -y cuda-drivers-fabricmanager cuda"
    ]
  }
